(function (fs, net, x2js, child_process, onddclient, telemetry, merge) {

  var conf_file= '/etc/rxos_config.json';
  var conf_file_countries= '/etc/crda_country_list.json';
  var conf_file_writeable_path= '/mnt/conf/etc/';
  var conf_file_writeable= conf_file_writeable_path + 'rxos_config.json';
  ...
  module.exports.getConf = function (name,cb) {
    fs.readFile(conf_file, function(error, data) {
      if(error)
        ...
        fs.readFile(conf_file_countries, function(error, cdata) {
          if(error)
            cb(null);
          else {
            var cdata_j = JSON.parse(cdata.toString());
            data_j.netConf.ap.countries = cdata_j.countries;
