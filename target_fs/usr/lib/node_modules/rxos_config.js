(function (fs, net, x2js, child_process, onddclient, telemetry) {

  var conf_file= '/etc/rxos_config.json';
  var conf_file_writeable= '/mnt/conf/etc/rxos_config.json';
  ...
  module.exports.getConf = function (name,cb) {
    fs.readFile(conf_file, function(error, data) {
      if(error)
        ...
        fs.readFile(conf_file_writable, function(error, data) {
            if (!error) {
                data_j_writeable = JSON.parse(data.toString());
                Object.assign(data_j, data_j_writable);
            }
            if (name) {
