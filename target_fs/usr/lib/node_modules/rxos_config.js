(function (fs, net, x2js, child_process) {
  ...
  var x = new x2js;
  module.exports.getOnddStatus2 = function(callback) {
    var status = { response : {} };
    module.exports.getConf("ondd",function (onddConf) {
      net.createConnection(onddConf.controlSocket)
      .setEncoding('utf8')
      .on('data', function(data) {
          var data_j = x.xml2js(data);
          if(data_j && data_j.response && status_j.response._code == 200) {
              if(data_j.response.tuner)
                status.response.tuner = data_j.response.tuner;
          }
      })
      .on('end', function() {
          net.createConnection(onddConf.controlSocket)
          ...
          .on('data', function(data) {
              var data_j = x.xml2js(data);
              if(data_j && data_j.response && status_j.response._code == 200) {
                if (data_j.response.streams)
                    status.response.transfers = data_j.response.streams.stream.transfers;
              }
          })
          .on('end', function() {
              if (status.response.tuner && status.response.transfers)
                   callback(status);
              else
                   callback(null);
          })
  ...
  module.exports.getOnddStatus2 = function(callback) {
    ...
    module.exports.getConf("ondd",function (onddConf) {
      ...
      .on('data', function(data) {
          ...
          var data_j = x.xml2js(data);
         console.log(data_j);
          if(data_j && data_j.response && data_j.response._code == 200) {
              if(data_j.response.tuner)
                status.response.tuner = data_j.response.tuner;
          net.createConnection(onddConf.controlSocket)
          .setEncoding('utf8')
          .on('data', function(data) {
              ...
              if(data_j && data_j.response && data_j.response._code == 200) {
                if (data_j.response.streams)
                    status.response.transfers = data_j.response.streams;
